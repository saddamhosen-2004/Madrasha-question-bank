  ...
  return (
    <div className="pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          <div className="card" style={{ padding: '24px' }}>
            ...
            <div style={{ background: '#f0f9f4', padding: '18px', borderRadius: '12px', border: '1.5px solid #c8e6d0' }}>
              <h3 style={{ fontBold: 700, color: 'var(--color-primary)', fontSize: '1rem', margin: '0 0 8px' }}>বার্ষিক সাবস্ক্রিপশন ফি: ৳৮,০০০</h3>
          ...
          <div className="card" style={{ padding: '24px' }}>
            ...
            <form onSubmit={handleSubmitPayment}>
              ...
              <div className="form-group" style={{ marginBottom: '18px' }}>
                ...
                <input type="text" className="input" style={{ background: '#f1f5f9', cursor: 'not-allowed', color: '#64748b' }} value="৳৮,০০০" disabled />
              </div>
              ...
              <button type="submit" disabled={submitting} className="btn btn-primary w-full" style={{ width: '100%', padding: '12px', justifyContent: 'center', fontWeight: 700, fontSize: '0.95rem' }} id="submit-payment-btn">
        ...
        {/* History */}
        <div>
          <div className="card h-full flex flex-col" style={{ minHeight: '440px' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
              <h2 className="text-xl font-bold" style={{ margin: 0 }}>পেমেন্ট হিস্ট্রি</h2>
            </div>
            <div className="flex-1 overflow-y-auto">
              {payments.length > 0 ? (
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-[var(--color-surface-2)] border-b border-[var(--color-border)]">
                        <th className="p-4" style={{ padding: '14px 16px' }}>তারিখ</th>
                        <th className="p-4" style={{ padding: '14px 16px' }}>মেথড</th>
                        <th className="p-4" style={{ padding: '14px 16px' }}>অ্যামাউন্ট</th>
                        <th className="p-4" style={{ padding: '14px 16px' }}>স্ট্যাটাস</th>
                      </tr>
                    </thead>
                    <tbody>
                      {payments.map(payment => (
                        <tr key={payment.id} className="border-b border-[var(--color-border)] last:border-0 text-[var(--color-text)]">
                          <td style={{ padding: '14px 16px' }}>{new Date(payment.created_at).toLocaleDateString('bn-BD')}</td>
                          <td style={{ padding: '14px 16px', textTransform: 'capitalize', fontWeight: 600 }}>{payment.payment_method}</td>
                          <td style={{ padding: '14px 16px' }}>৳{payment.amount}</td>
                ...
                <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
